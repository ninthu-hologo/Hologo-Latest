...
#pragma exclude_renderers d3d11 gles
		...
		float3 Tex2DprojBlurred( sampler2D tex, float4 coord, float blur ) {
			...
			int horizontal[8] = {0,0,0,0,0,0,0,0};
			int vertical[8] = {0,0,0,0,0,0,0,0};
			for (int x = 0; x < 8; x++) {
				horizontal[x] = tex2Dproj(
					tex, 
					coord + float4(
						((float)x/ _ScreenParams.x)*blur,
						0,
						0,0
					)
				).rgb;
			...
			for (int y = 0; y < 8; y++){
				vertical[y] = tex2Dproj(
					tex, 
					coord + float4(
						0,
						((float)y/ _ScreenParams.y)*blur,
						0,0
					)
				).rgb;
			}
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float d = Tex2DprojBlurred(_CameraDepthTexture, IN.screenPos + float4(-dispVec*(_IOR - 1)*0.5, 0, 0), blur).r;
				if (_CA <= 0) {
					dispBG = Tex2DprojBlurred(_GrabTexture, IN.screenPos, blur).rgb;
				}
				else {
